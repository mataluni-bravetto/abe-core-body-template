...
test('SubscriptionService getDefaultUsage returns unlimited', () => {
  ...
  assertTrue(defaultUsage.requests_limit === null, 'Default requests limit should be null (unlimited)');
...
test('SubscriptionService getTierDisplayName formats correctly', () => {
  ...
  assertEqual(subscriptionService.getTierDisplayName('free'), 'Free', 'Free tier should format correctly');
  assertEqual(subscriptionService.getTierDisplayName('pro'), 'Pro', 'Pro tier should format correctly');
  assertEqual(subscriptionService.getTierDisplayName('enterprise'), 'Enterprise', 'Enterprise tier should format correctly');
  assertEqual(subscriptionService.getTierDisplayName('unknown'), 'unknown', 'Unknown tier should return as-is');
});
...
test('SubscriptionService getStatusDisplayName formats correctly', () => {
  ...
  assertEqual(subscriptionService.getStatusDisplayName('active'), 'Active', 'Active status should format correctly');
  assertEqual(subscriptionService.getStatusDisplayName('cancelled'), 'Cancelled', 'Cancelled status should format correctly');
  assertEqual(subscriptionService.getStatusDisplayName('expired'), 'Expired', 'Expired status should format correctly');
  assertEqual(subscriptionService.getStatusDisplayName('trialing'), 'Trial', 'Trialing status should format correctly');
});
...
test('SubscriptionService clearCache resets cache', () => {
  ...
  assertTrue(subscriptionService.cache.subscription === null, 'Subscription cache should be cleared');
...
test('SubscriptionService canMakeRequest fails open on error', async () => {
  ...
  assertTrue(result.message.includes('Unable to verify'), 'Message should indicate verification failure');
});
