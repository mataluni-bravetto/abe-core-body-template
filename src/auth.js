...
class AiGuardianAuth {
  ...
  async initialize() {
    try {
      ...
      this.publishableKey = settings.clerk_publishable_key
        ? settings.clerk_publishable_key.trim()
        : null;
      Logger.info(
        '[Auth] Got settings, key present:',
        !!this.publishableKey,
        'source:',
        settings.source
      );
      ...
      if (!this.publishableKey) {
        Logger.error(
          '[Auth] Clerk publishable key not found - must be configured via backend API or options page'
        );
        throw new Error(
          'Clerk publishable key not configured. Please configure it in extension settings or ensure backend API returns it.'
        );
      }
      ...
      if (typeof this.clerk.load === 'function') {
        ...
        if (typeof this.clerk.user !== 'undefined' || typeof this.clerk.session !== 'undefined') {
          ...
          Logger.warn(
            '[Auth] Clerk SDK loaded but load() method not available - may need different initialization'
          );
        }
      }
  ...
  async fetchPublicConfig() {
    try {
      ...
      if (config.clerk_publishable_key) {
        Logger.info(
          `[Auth] Successfully fetched Clerk publishable key from backend (source: ${config.source || 'unknown'})`
        );
      ...
      return {
        error: 'Backend returned empty response - no Clerk key',
  ...
  buildClerkInstanceUrl(publishableKey, path, redirectUrl) {
    ...
    try {
      ...
      const instanceDomain =
        keyType === 'test' ? `${instanceId}.accounts.dev` : `${instanceId}.clerk.accounts.dev`;
    ...
    return new Promise((resolve) => {
      ...
      chrome.storage.sync.get(['clerk_publishable_key'], (data) => {
        ...
        if (manualKey) {
          ...
          const hardcodedKey =
            typeof DEFAULT_CONFIG !== 'undefined' && DEFAULT_CONFIG.CLERK_PUBLISHABLE_KEY
  ...
  async signIn() {
    ...
    Logger.info(
      '[Auth] publishableKey:',
      this.publishableKey ? `${this.publishableKey.substring(0, 20)}...` : 'null'
    );
    ...
    if (!this.publishableKey) {
      ...
      if (!this.publishableKey) {
        throw new Error(
          'Clerk publishable key not configured. Please configure it in extension settings.'
        );
      }
    }
      ...
      if (this.publishableKey) {
        ...
        if (this.clerk && this.clerk.client && typeof this.clerk.client.signIn === 'object') {
          try {
            ...
            Logger.warn(
              '[Auth] Failed to generate URL from Clerk SDK, using manual construction:',
              sdkError
            );
          }
        }
